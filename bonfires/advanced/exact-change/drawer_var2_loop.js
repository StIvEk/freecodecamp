...
function drawer(price, cash, cid) {
  ...
  function calcChange() {
    ...
    for (var i = 0; i < currencies.length; i++) {
      currencyItemsNeeded = Math.floor(changeRemaining / currencyAmounts[currencies[i][0]]);
      if (!currencyItemsNeeded) continue;

      currencyAmountNeeded = currencyItemsNeeded * currencyAmounts[currencies[i][0]];
      currencyAmountReturned = Math.min(currencyAmountNeeded, currencies[i][1]);
      changeRemaining -= currencyAmountReturned;
      changeRemaining = parseFloat(changeRemaining.toFixed(2));
      currenciesToReturn.push([currencies[i][0], parseFloat(currencyAmountReturned)]);
    }
